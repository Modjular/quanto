// fastfilters3
// Tony -- 20260602
// My javascript version of fastfilters2, ported to WebGPU

export const fastfilters3 = {
  // Feature IDs exactly matching ilastik/run_pc.py for backwards compatibility.
  // Note: DifferenceOfGaussians is omitted from this single-scale 7-channel stack.
  FEATURE_IDS: [
    "GaussianSmoothing",
    "LaplacianOfGaussian",
    "GaussianGradientMagnitude",
    "StructureTensorEigenvalues",   // 2 channels
    "HessianOfGaussianEigenvalues"  // 2 channels
  ],
  ...
  /**
   * For efficiency, calculates all features in a single pass
   * @param {*} device 
   * @param {*} data 
   * @param {*} width 
   * @param {*} height 
   ...
   * [3, 4] StructureTensorEigenvalues (Largest, Smallest)                                                        
   * [5, 6] HessianOfGaussianEigenvalues (Largest, Smallest) 
   */
  async compute_filters(device, data, width, height, scale) {
    ...
    const gpuInput = device.createBuffer({ size: data.byteLength, usage: 0x0080 | 0x0008 }); // STORAGE | COPY_DST
    device.queue.writeBuffer(gpuInput, 0, data);
    const gpuHoriz = device.createBuffer({ size: width * height * 3 * 4, usage: 0x0080 }); // STORAGE
    const gpuOutput = device.createBuffer({ size: outSize * 4, usage: 0x0080 | 0x0004 }); // STORAGE | COPY_SRC
    ...
    const horizShader = `
      ...
      fn main(@builtin(global_invocation_id) id: vec3<u32>) {
        ...
        let out_idx = (u32(y) * ${width}u + u32(x)) * 3u;
        horiz_data[out_idx] = h0; horiz_data[out_idx + 1] = h1; horiz_data[out_idx + 2] = h2;
      }
    `;
    ...
    const vertShader = `
      ...
      fn get_h(ix: i32, iy: i32) -> vec3<f32> {
        let idx = (u32(clamp(iy, 0, ${height - 1})) * ${width}u + u32(clamp(ix, 0, ${width - 1}))) * 3u;
      ...
      fn main(@builtin(global_invocation_id) id: vec3<u32>) {
        ...
        let out_idx = (u32(y) * ${width}u + u32(x)) * 7u;
        
        // Order matching ilastik FeatureIds concatenation:
        // --- 1. GaussianSmoothing ---
        output_data[out_idx] = L; 
        
        // --- 2. LaplacianOfGaussian ---
        output_data[out_idx + 1] = Lxx + lyy; 
        
        // --- 3. GaussianGradientMagnitude ---
        output_data[out_idx + 2] = sqrt(Lx*Lx + Ly*Ly);

        // --- 4. StructureTensorEigenvalues (Largest, Smallest) ---
        let s_a = Lx*Lx; let s_b = Lx*Ly; let s_c = Ly*Ly;
        let s_term = sqrt((s_a - s_c)*(s_a - s_c) * 0.25 + s_b*s_b);
        output_data[out_idx + 3] = (s_a + s_c) * 0.5 + s_term; 
        output_data[out_idx + 4] = (s_a + s_c) * 0.5 - s_term;

        // --- 5. HessianOfGaussianEigenvalues (Largest, Smallest) ---
        let h_term = sqrt((Lxx - lyy)*(Lxx - lyy) * 0.25 + Lxy*Lxy);
        output_data[out_idx + 5] = (Lxx + lyy) * 0.5 + h_term; 
        output_data[out_idx + 6] = (Lxx + lyy) * 0.5 - h_term;
      }
    `;
    ...
    const rb = device.createBuffer({ size: outSize * 4, usage: 0x0008 | 0x0001 }); // COPY_DST | MAP_READ
